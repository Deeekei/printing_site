...
class CatalogView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if querry:
            ...
            products.filter(
                Q(name__icontains=querry | Q(descriprion__icontains=querry))
            )    
        filter_params = {}
        ...
        context.update({
            ...
            'search_querry': querry or ''
        })

        if self.request.GET('show_search') == 'true':
            context['show_search'] = True
        elif self.request.GET('reset_search') == 'true':
...
'''
    FILTER_MAPPING = {
        'color': lambda querryset, value: querryset.filter(color_iexact=value)
        'color': lambda querryset, value: querryset.filter(color_iexact=value)
        'color': lambda querryset, value: querryset.filter(color_iexact=value)
